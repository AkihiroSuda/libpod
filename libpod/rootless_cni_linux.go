...
var rootlessCNIInfraImage = map[string]string{
	...
	"amd64": "ghcr.io/akihirosuda/podman-rootless-cni-infra:g19311bd63-amd64",
}
...
func rootlessCNIInfraCallAlloc(infra *Container, id, nw, k8sPodName string) (*cnitypes.Result, error) {
	...
	_, err = execOutput(infra, true, "rootless-cni-infra", "alloc", id, nw, k8sPodName)
	if err != nil {
		return nil, err
	}
	cniResStr, err := execOutput(infra, false, "rootless-cni-infra", "print-cni-result", id, nw)
	if err != nil {
		return nil, err
	}
...
func rootlessCNIInfraCallDelloc(infra *Container, id, nw string) error {
	...
	_, err := execOutput(infra, true, "rootless-cni-infra", "dealloc", id, nw)
	return err
}
...
func rootlessCNIInfraIsIdle(infra *Container) (bool, error) {
	...
	resStr, err := execOutput(infra, false, "rootless-cni-infra", "is-idle")
	if err != nil {
		return false, err
	}
	var res isIdle
...
func rootlessCNIInfraGetNS(infra *Container, id string) (ns.NetNS, error) {
	...
	resStr, err := execOutput(infra, false, "rootless-cni-infra", "print-netns-path", id)
	if err != nil {
		return nil, err
	}
...
func execOutput(c *Container, privileged bool, cmd string, args ...string) (string, error) {
	var (
		outB    bytes.Buffer
		errB    bytes.Buffer
	...
	logrus.Debugf("execOutput: c.ID()=%s, config=%+v, streams=%v, begin",
		c.ID(), config, streams)
	code, err := c.Exec(&config, &streams, nil)
	logrus.Debugf("execOutput: c.ID()=%s, config=%+v, streams=%v, end (code=%d, err=%v)",
